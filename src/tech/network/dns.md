...
### Intro
DNS, `Domain Name System`，域名系统，是互联网的一项服务。它作为将域名和IP
地址相互映射的一个分布式数据库，能够使人更方便地访问互联网。

DNS 中的域名都是用**句点**来分隔的，比如 `www.server.com`，这里的句点代表了不同层次之间的**界限**。在域名中，**越靠右**的位置表示其层级**越高**。

域名的类型：
- 根域名：它不包含在域名中，但是它是域名解析过程中**隐含的最高层级**。
- 顶级域名：如`com`、`cn`、`org`等
- 二级域名/权威域名：如`baidu.com`、`google.com`等
...
- 查找本地hosts文件是否存在解析记录
- 查找本地DNS服务器是否存在缓存 (校园网不让你访问某些网站可能就是在这一步拦截的)
...
注意，前部分
从浏览器缓存、host文件再到本地DNS服务器，这部分是递归查询。
本地DNS服务器依次询问根域名服务器、顶级、权威...这是迭代(遍历)查询。
